...
let currentIndex = 0
arrayItems[currentIndex].classList.add(...axtiveStyles, 'text-white')
arrayItems.forEach((element, index) => {
  element.addEventListener('click', () => {
    ...
    arrayItems[currentIndex].classList.remove(...axtiveStyles, 'text-white')
    currentIndex = index
    if (currentIndex > 0) {
      arrayItems[currentIndex - 1].classList.remove('border-b-2')
    }
    element.classList.add(...axtiveStyles, 'text-white')
  });
});
